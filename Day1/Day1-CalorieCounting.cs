using advent_of_code_2022.Utility;
...
static void CaloriesCarriedByElf(List<string> input)
{
    ...
    var caloriesTotal = new List<int>();

    foreach (string s in input)
    {
        if (s == "")
        {
            caloriesTotal.Add(totalCalculation);
            totalCalculation = 0;
        }
        else
        {
            totalCalculation += int.Parse(s);
        }
    }
    caloriesTotal = caloriesTotal.OrderByDescending(x => x).ToList();
